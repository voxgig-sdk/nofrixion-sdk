...
describe('PaymentAccountMinimalDirect', async () => {
  ...
  test('direct-list-payment_account_minimal', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-payment_account_minimal', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_PAYMENT_ACCOUNT_MINIMAL_ENTID']
