...
describe('MerchantPayByBankSettingDirect', async () => {
  ...
  test('direct-list-merchant_pay_by_bank_setting', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-merchant_pay_by_bank_setting', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_MERCHANT_PAY_BY_BANK_SETTING_ENTID']
