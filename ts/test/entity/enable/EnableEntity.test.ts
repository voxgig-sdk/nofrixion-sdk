...
describe('EnableEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    const enable_ref01_data_up0: any = {}
    enable_ref01_data_up0.id = enable_ref01_data.id

    const enable_ref01_markdef_up0 = { name: 'approval_callback_url', value: 'Mark01-enable_ref01_' + setup.now }
    ;(enable_ref01_data_up0 as any)[enable_ref01_markdef_up0.name] = enable_ref01_markdef_up0.value

    const enable_ref01_resdata_up0 = await enable_ref01_ent.update(enable_ref01_data_up0)
    assert(enable_ref01_resdata_up0.id === enable_ref01_data_up0.id)

    assert((enable_ref01_resdata_up0 as any)[enable_ref01_markdef_up0.name] === enable_ref01_markdef_up0.value)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['NOFRIXION_TEST_ENABLE_ENTID']
  ...
  idmap = env['NOFRIXION_TEST_ENABLE_ENTID']
