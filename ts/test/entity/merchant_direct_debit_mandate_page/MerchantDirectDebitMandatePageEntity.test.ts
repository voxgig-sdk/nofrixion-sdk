...
describe('MerchantDirectDebitMandatePageEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'merchant_direct_debit_mandate_page.' + op, live)) return
    }
    ...
    let merchant_direct_debit_mandate_page_ref01_data = Object.values(setup.data.existing.merchant_direct_debit_mandate_page)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/merchant_direct_debit_mandate_page/MerchantDirectDebitMandatePageTestData.json')
  ...
  const idmapEnvVal = process.env['NOFRIXION_TEST_MERCHANT_DIRECT_DEBIT_MANDATE_PAGE_ENTID']
