...
describe('MerchantDirectDebitMandatePageDirect', async () => {
  ...
  test('direct-list-merchant_direct_debit_mandate_page', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-merchant_direct_debit_mandate_page', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_MERCHANT_DIRECT_DEBIT_MANDATE_PAGE_ENTID']
