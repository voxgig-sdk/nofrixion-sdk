...
describe('CardCustomerTokenDirect', async () => {
  ...
  test('direct-load-card_customer_token', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-card_customer_token', setup.live)) return
  ...
  test('direct-list-card_customer_token', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-card_customer_token', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_CARD_CUSTOMER_TOKEN_ENTID']
