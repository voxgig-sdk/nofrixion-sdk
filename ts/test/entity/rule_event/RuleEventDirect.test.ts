...
describe('RuleEventDirect', async () => {
  ...
  test('direct-list-rule_event', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-rule_event', setup.live)) return
    if (skipIfMissingIds(t, setup, ["rule_event01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_RULE_EVENT_ENTID']
