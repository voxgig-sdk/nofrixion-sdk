...
describe('SendEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'send.' + op, live)) return
    }
    ...
    // CREATE
    const send_ref01_ent = client.Send()
    let send_ref01_data = setup.data.new.send['send_ref01']

    send_ref01_data = await send_ref01_ent.create(send_ref01_data)
    assert(null != send_ref01_data.id)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/send/SendTestData.json')
  ...
  const idmapEnvVal = process.env['NOFRIXION_TEST_SEND_ENTID']
  ...
  idmap = env['NOFRIXION_TEST_SEND_ENTID']
