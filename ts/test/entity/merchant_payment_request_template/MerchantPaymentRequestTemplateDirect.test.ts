...
describe('MerchantPaymentRequestTemplateDirect', async () => {
  ...
  test('direct-load-merchant_payment_request_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-merchant_payment_request_template', setup.live)) return
    if (skipIfMissingIds(t, setup, ["paymentrequest01"])) return
  ...
  test('direct-list-merchant_payment_request_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-merchant_payment_request_template', setup.live)) return
    if (skipIfMissingIds(t, setup, ["merchant01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_MERCHANT_PAYMENT_REQUEST_TEMPLATE_ENTID']
