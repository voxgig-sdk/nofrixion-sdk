...
describe('PaymentRequestMetricDirect', async () => {
  ...
  test('direct-load-payment_request_metric', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-payment_request_metric', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NOFRIXION_TEST_PAYMENT_REQUEST_METRIC_ENTID']
