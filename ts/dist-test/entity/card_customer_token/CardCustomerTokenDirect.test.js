...
(0, node_test_1.describe)('CardCustomerTokenDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-card_customer_token', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-card_customer_token', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["customer_email_address01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.customer_email_address ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.customer_email_address = candidateId;
    ...
    (0, node_test_1.test)('direct-list-card_customer_token', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-card_customer_token', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["customer_email_address01", "merchant01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['NOFRIXION_TEST_CARD_CUSTOMER_TOKEN_ENTID'];
...
//# sourceMappingURL=CardCustomerTokenDirect.test.js.map