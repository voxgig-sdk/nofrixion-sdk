...
(0, node_test_1.describe)('FxRateDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.NofrixionSDK({
    ...
    (0, node_test_1.test)('direct-load-fx_rate', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-fx_rate', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["destination01", "source01", "valid_for_minute01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.destination ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.destination = candidateId;
    ...
    (0, node_test_1.test)('direct-list-fx_rate', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-fx_rate', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["destination01", "source01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.NOFRIXION_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'NOFRIXION_TEST_FX_RATE_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.NofrixionSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.NOFRIXION_APIKEY,
        }));
        let idmap = env['NOFRIXION_TEST_FX_RATE_ENTID'];
    ...
    const client = new __1.NofrixionSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=FxRateDirect.test.js.map