...
(0, node_test_1.describe)('MerchantDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-merchant', async (t) => {
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.merchant_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.merchant_id = candidateId;
        ...
        const result = await client.direct({
            path: 'api/v1/merchants/{merchant_id}/payouts/export',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['NOFRIXION_TEST_MERCHANT_ENTID'];
