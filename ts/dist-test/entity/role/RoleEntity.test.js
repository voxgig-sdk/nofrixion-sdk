...
(0, node_test_1.describe)('RoleEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['create']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'role.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_ROLE_ENTID JSON to run live');
            return;
        }
        ...
        (0, node_assert_1.default)(null != role_ref01_data);
    });
});
function basicSetup(extra) {
    ...
    let idmap = transform(['role01', 'role02', 'role03', 'merchant01', 'merchant02', 'merchant03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['NOFRIXION_TEST_ROLE_ENTID'];
    ...
    idmap = env['NOFRIXION_TEST_ROLE_ENTID'];
