...
| `consolidatedAccountInformation` | `Record<string, any>` | No | Summary information regarding account balances of the overall account provided by the bank. |
...
```ts
const result = await client.CardCustomerToken().remove({ id: 'id' })
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `failedSubmissions` | `Record<string, any>` | No | Dictionary of failed submissions, keyed by the index (1-based) in the original request. |
...
```ts
const result = await client.Merchant().remove({ id: 'merchant_id', user_id: 'user_id' })
```
...
```ts
const result = await client.OpenBanking().remove({ account_id: 'account_id' })
```
...
| `directDebitPayment` | `Record<string, any>` | No | Contains information about a Direct Debit payment attempt for a payment request. |
...
```ts
const results = await client.PayoutKeysetPage().list({ merchant_id: "example" })
```
...
| `currencyExchange` | `Record<string, any>` | No | Provides details on the currency exchange. |
| `date` | `string` | No |  |
...
| `payeeDetails` | `Record<string, any>` | Yes | The Payee object contains details of the beneficiary, person or business. |
| `payerDetails` | `Record<string, any>` | Yes |  |
| `paymentRequestCustomFields` | `Record<string, any>` | No | The custom fields that were attached to the payment request that resulted in this transaction. |
...
```ts
const results = await client.Transaction().list({ account_id: "example", id: "example_id" })
```
