...
class NofrixionTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NofrixionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NofrixionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NofrixionTestRunner.getenv("NOFRIXION_TEST_LIVE")
        override = NofrixionTestRunner.getenv("NOFRIXION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NofrixionTestRunner.getenv(key)
        ...
        explain = NofrixionTestRunner.getenv("NOFRIXION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NOFRIXION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NofrixionTestRunner._test_control is not None:
            return NofrixionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NofrixionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NofrixionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NofrixionTestRunner.load_test_control()
...
def load_env_local():
    NofrixionTestRunner.load_env_local()


def env_override(m):
    return NofrixionTestRunner.env_override(m)


def entity_data(v):
    return NofrixionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NofrixionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NofrixionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NofrixionTestRunner.load_test_control()


def live_client_options():
    return NofrixionTestRunner.live_client_options()


def live_delay_ms():
    return NofrixionTestRunner.live_delay_ms()
