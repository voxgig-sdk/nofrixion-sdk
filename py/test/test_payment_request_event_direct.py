...
class TestPaymentRequestEventDirect:

    def test_should_direct_list_payment_request_event(self):
        setup = _payment_request_event_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-payment_request_event", "live" if setup["live"] else "unit")
        if _skip:
...
def _payment_request_event_direct_setup(mockres):
