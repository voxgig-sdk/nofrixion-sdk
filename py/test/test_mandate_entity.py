...
class TestMandateEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Mandate(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _mandate_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "mandate." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NOFRIXION_TEST_MANDATE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        mandate_ref01_ent = client.Mandate(None)
...
def _mandate_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/mandate/MandateTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NOFRIXION_TEST_MANDATE_ENTID"))
