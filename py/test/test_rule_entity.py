...
class TestRuleEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Rule(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Rule(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _rule_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "rule." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NOFRIXION_TEST_RULE_ENTID JSON to run live")
        client = setup["client"]
        ...
        rule_ref01_ent.remove(rule_ref01_match_rm0, None)

        # LIST
        rule_ref01_match_rt0 = {}

        rule_ref01_list_rt0_result = rule_ref01_ent.list(rule_ref01_match_rt0, None)
        assert isinstance(rule_ref01_list_rt0_result, list)
...
def _rule_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/rule/RuleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NOFRIXION_TEST_RULE_ENTID"))
