...
class TestMerchantTokenEntity:

    def test_should_create_instance(self):
        testsdk = NofrixionSDK.test(None, None)
        ent = testsdk.MerchantToken(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.MerchantToken(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.MerchantToken(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _merchant_token_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "merchant_token." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NOFRIXION_TEST_MERCHANT_TOKEN_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        merchant_token_ref01_ent = client.MerchantToken(None)
        ...
        merchant_token_ref01_list_result = merchant_token_ref01_ent.list(merchant_token_ref01_match, None)
        assert isinstance(merchant_token_ref01_list_result, list)
...
def _merchant_token_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/merchant_token/MerchantTokenTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NOFRIXION_TEST_MERCHANT_TOKEN_ENTID"))
