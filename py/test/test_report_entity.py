...
class TestReportEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Report(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _report_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "report." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NOFRIXION_TEST_REPORT_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        report_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.report")))
        report_ref01_data = None
        if len(report_ref01_data_raw) > 0:
...
def _report_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/report/ReportTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NOFRIXION_TEST_REPORT_ENTID"))
