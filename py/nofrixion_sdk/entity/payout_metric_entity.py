...
class PayoutMetricEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PayoutMetricEntity(self._client, opts)
    ...
    def data_get(self) -> PayoutMetric:
    ...
    def match_get(self) -> PayoutMetric:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PayoutMetric:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PayoutMetric().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
