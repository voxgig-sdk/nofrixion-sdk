...
class OpenBankingEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OpenBankingEntity(self._client, opts)
    ...
    def match_get(self) -> OpenBanking:
    ...
    def create(self, reqdata: OpenBankingCreateData, ctrl=None) -> OpenBanking:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> OpenBanking:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.OpenBanking().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
