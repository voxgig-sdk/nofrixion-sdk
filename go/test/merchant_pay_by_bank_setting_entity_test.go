...
func TestMerchantPayByBankSettingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.MerchantPayByBankSetting(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.MerchantPayByBankSetting(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		merchantPayByBankSettingRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.merchant_pay_by_bank_setting")))
