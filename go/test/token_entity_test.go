...
func TestTokenEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_TOKEN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tokenRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "token"}), "token_ref01"))
...
func tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_TOKEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_TOKEN_ENTID"])
