...
import (
	...
	omni "github.com/voxgig-sdk/nofrixion-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NofrixionError); is {
				out := map[string]any{
					"name":    "NofrixionError",
...
func omniCtx(arg any, client *sdk.NofrixionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NofrixionSDK); is {
			client = live
