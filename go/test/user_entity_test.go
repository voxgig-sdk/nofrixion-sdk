...
func userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"user01", "user02", "user03", "merchant01", "merchant02", "merchant03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_USER_ENTID"])
