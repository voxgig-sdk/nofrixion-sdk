...
func TestDisableEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_DISABLE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		disableRef01DataUp0Up := map[string]any{
			"id": disableRef01Data["id"],
		}

		disableRef01MarkdefUp0Name := "approval_callback_url"
		disableRef01MarkdefUp0Value := fmt.Sprintf("Mark01-disable_ref01_%d", setup.now)
		disableRef01DataUp0Up[disableRef01MarkdefUp0Name] = disableRef01MarkdefUp0Value
		...
		if disableRef01ResdataUp0["id"] != disableRef01DataUp0Up["id"] {
			t.Fatal("expected update result id to match")
		}
		if disableRef01ResdataUp0[disableRef01MarkdefUp0Name] != disableRef01MarkdefUp0Value {
			t.Fatalf("expected %s to be updated, got %v", disableRef01MarkdefUp0Name, disableRef01ResdataUp0[disableRef01MarkdefUp0Name])
		}
...
func disableBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_DISABLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_DISABLE_ENTID"])
