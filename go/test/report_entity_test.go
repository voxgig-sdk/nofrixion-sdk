...
func TestReportEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_REPORT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		reportRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.report", setup.data)))
		var reportRef01Data map[string]any
		if len(reportRef01DataRaw) > 0 {
			reportRef01Data = core.ToMapAny(reportRef01DataRaw[0][1])
		}
		...
		reportRef01ResdataUp0 := core.ToMapAny(reportRef01ResdataUp0Result)
...
func reportBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_REPORT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_REPORT_ENTID"])
