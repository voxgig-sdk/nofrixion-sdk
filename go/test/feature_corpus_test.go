...
//
// NewNofrixionSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.NofrixionSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewNofrixionSDK(opts)
}
...
func fcCandidates(client *sdk.NofrixionSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.NofrixionSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.NofrixionSDK, name string) any {
