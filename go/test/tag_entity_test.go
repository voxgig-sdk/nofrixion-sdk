...
func TestTagEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_TAG_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		tagRef01Data["merchant_id"] = setup.idmap["merchant01"]
...
func tagBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"tag01", "tag02", "tag03", "merchant01", "merchant02", "merchant03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_TAG_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_TAG_ENTID"])
