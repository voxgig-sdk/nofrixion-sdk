...
func TestMetadataEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_METADATA_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func metadataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_METADATA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_METADATA_ENTID"])
