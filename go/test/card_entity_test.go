...
func TestCardEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CARD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		cardRef01Data["paymentrequest_id"] = setup.idmap["paymentrequest01"]
...
func cardBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"card01", "card02", "card03", "paymentrequest01", "paymentrequest02", "paymentrequest03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("NOFRIXION_TEST_CARD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NOFRIXION_TEST_CARD_ENTID"])
