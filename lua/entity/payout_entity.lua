...
local PayoutEntity = {}
PayoutEntity.__index = PayoutEntity


function PayoutEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PayoutEntity)
  self._name = "payout"
  self._client = client
...
function PayoutEntity:get_name()
  return self._name
end


function PayoutEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PayoutEntity.new(self._client, opts)
end
...
function PayoutEntity:mark_deleted()
  self._deleted = true
end


function PayoutEntity:deleted()
  return true == self._deleted
end


function PayoutEntity:data_set(args)
  if args ~= nil then
...
function PayoutEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PayoutEntity:match_set(args)
  if args ~= nil then
...
function PayoutEntity:match_get()
...
function PayoutEntity:stream(action, args, callopts)
...
---@param reqmatch PayoutLoadMatch
---@param ctrl? table
---@return Payout
---@return string? err
function PayoutEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PayoutListMatch
---@param ctrl? table
---@return Payout[]
---@return string? err
function PayoutEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata PayoutCreateData
---@param ctrl? table
---@return Payout
---@return string? err
function PayoutEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata PayoutUpdateData
---@param ctrl? table
---@return Payout
---@return string? err
function PayoutEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch PayoutRemoveMatch
---@param ctrl? table
---@return Payout
---@return string? err
function PayoutEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function PayoutEntity:_run_op(ctx, post_done)
  local utility = self._utility
