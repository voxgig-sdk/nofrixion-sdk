-- Nofrixion SDK CardPayment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CardPaymentEntity = {}
CardPaymentEntity.__index = CardPaymentEntity


function CardPaymentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardPaymentEntity)
  self._name = "card_payment"
  self._client = client
...
function CardPaymentEntity:get_name()
  return self._name
end


function CardPaymentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardPaymentEntity.new(self._client, opts)
end
...
function CardPaymentEntity:mark_deleted()
  self._deleted = true
end


function CardPaymentEntity:deleted()
  return true == self._deleted
end


function CardPaymentEntity:data_set(args)
  if args ~= nil then
...
function CardPaymentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardPaymentEntity:match_set(args)
  if args ~= nil then
...
function CardPaymentEntity:match_get()
...
function CardPaymentEntity:stream(action, args, callopts)
...
---@param reqdata CardPaymentCreateData
---@param ctrl? table
---@return CardPayment
---@return string? err
function CardPaymentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CardPaymentEntity:_run_op(ctx, post_done)
  local utility = self._utility
