-- Nofrixion SDK ReportResult entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReportResultEntity = {}
ReportResultEntity.__index = ReportResultEntity


function ReportResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportResultEntity)
  self._name = "report_result"
  self._client = client
...
function ReportResultEntity:get_name()
  return self._name
end


function ReportResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportResultEntity.new(self._client, opts)
end
...
function ReportResultEntity:mark_deleted()
  self._deleted = true
end


function ReportResultEntity:deleted()
  return true == self._deleted
end


function ReportResultEntity:data_set(args)
  if args ~= nil then
...
function ReportResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportResultEntity:match_set(args)
  if args ~= nil then
...
function ReportResultEntity:match_get()
...
function ReportResultEntity:stream(action, args, callopts)
...
---@param reqmatch ReportResultLoadMatch
---@param ctrl? table
---@return ReportResult
---@return string? err
function ReportResultEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ReportResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
