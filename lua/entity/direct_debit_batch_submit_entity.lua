-- Nofrixion SDK DirectDebitBatchSubmit entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DirectDebitBatchSubmitEntity = {}
DirectDebitBatchSubmitEntity.__index = DirectDebitBatchSubmitEntity


function DirectDebitBatchSubmitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DirectDebitBatchSubmitEntity)
  self._name = "direct_debit_batch_submit"
  self._client = client
...
function DirectDebitBatchSubmitEntity:get_name()
  return self._name
end


function DirectDebitBatchSubmitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DirectDebitBatchSubmitEntity.new(self._client, opts)
end
...
function DirectDebitBatchSubmitEntity:mark_deleted()
  self._deleted = true
end


function DirectDebitBatchSubmitEntity:deleted()
  return true == self._deleted
end


function DirectDebitBatchSubmitEntity:data_set(args)
  if args ~= nil then
...
function DirectDebitBatchSubmitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DirectDebitBatchSubmitEntity:match_set(args)
  if args ~= nil then
...
function DirectDebitBatchSubmitEntity:match_get()
...
function DirectDebitBatchSubmitEntity:stream(action, args, callopts)
...
---@param reqdata DirectDebitBatchSubmitCreateData
---@param ctrl? table
---@return DirectDebitBatchSubmit
---@return string? err
function DirectDebitBatchSubmitEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DirectDebitBatchSubmitEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DirectDebitBatchSubmitEntity
