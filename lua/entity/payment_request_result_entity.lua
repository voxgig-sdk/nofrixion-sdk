-- Nofrixion SDK PaymentRequestResult entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentRequestResultEntity = {}
PaymentRequestResultEntity.__index = PaymentRequestResultEntity


function PaymentRequestResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentRequestResultEntity)
  self._name = "payment_request_result"
  self._client = client
...
function PaymentRequestResultEntity:get_name()
  return self._name
end


function PaymentRequestResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentRequestResultEntity.new(self._client, opts)
end
...
function PaymentRequestResultEntity:mark_deleted()
  self._deleted = true
end


function PaymentRequestResultEntity:deleted()
  return true == self._deleted
end


function PaymentRequestResultEntity:data_set(args)
  if args ~= nil then
...
function PaymentRequestResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentRequestResultEntity:match_set(args)
  if args ~= nil then
...
function PaymentRequestResultEntity:match_get()
...
function PaymentRequestResultEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentRequestResultListMatch
---@param ctrl? table
---@return PaymentRequestResult[]
---@return string? err
function PaymentRequestResultEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentRequestResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PaymentRequestResultEntity
