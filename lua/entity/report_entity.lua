-- Nofrixion SDK Report entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReportEntity = {}
ReportEntity.__index = ReportEntity


function ReportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReportEntity)
  self._name = "report"
  self._client = client
...
function ReportEntity:get_name()
  return self._name
end


function ReportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReportEntity.new(self._client, opts)
end
...
function ReportEntity:mark_deleted()
  self._deleted = true
end


function ReportEntity:deleted()
  return true == self._deleted
end


function ReportEntity:data_set(args)
  if args ~= nil then
...
function ReportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReportEntity:match_set(args)
  if args ~= nil then
...
function ReportEntity:match_get()
...
function ReportEntity:stream(action, args, callopts)
...
---@param reqdata ReportUpdateData
---@param ctrl? table
---@return Report
---@return string? err
function ReportEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function ReportEntity:_run_op(ctx, post_done)
  local utility = self._utility
