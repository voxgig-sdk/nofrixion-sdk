-- Nofrixion SDK PaymentRequestEvent entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentRequestEventEntity = {}
PaymentRequestEventEntity.__index = PaymentRequestEventEntity


function PaymentRequestEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentRequestEventEntity)
  self._name = "payment_request_event"
  self._client = client
...
function PaymentRequestEventEntity:get_name()
  return self._name
end


function PaymentRequestEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentRequestEventEntity.new(self._client, opts)
end
...
function PaymentRequestEventEntity:mark_deleted()
  self._deleted = true
end


function PaymentRequestEventEntity:deleted()
  return true == self._deleted
end


function PaymentRequestEventEntity:data_set(args)
  if args ~= nil then
...
function PaymentRequestEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentRequestEventEntity:match_set(args)
  if args ~= nil then
...
function PaymentRequestEventEntity:match_get()
...
function PaymentRequestEventEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentRequestEventListMatch
---@param ctrl? table
---@return PaymentRequestEvent[]
---@return string? err
function PaymentRequestEventEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentRequestEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PaymentRequestEventEntity
