...
local FxRateEntity = {}
FxRateEntity.__index = FxRateEntity


function FxRateEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FxRateEntity)
  self._name = "fx_rate"
  self._client = client
...
function FxRateEntity:get_name()
  return self._name
end


function FxRateEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FxRateEntity.new(self._client, opts)
end
...
function FxRateEntity:mark_deleted()
  self._deleted = true
end


function FxRateEntity:deleted()
  return true == self._deleted
end


function FxRateEntity:data_set(args)
  if args ~= nil then
...
function FxRateEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FxRateEntity:match_set(args)
  if args ~= nil then
...
function FxRateEntity:match_get()
...
function FxRateEntity:stream(action, args, callopts)
...
---@param reqmatch FxRateLoadMatch
---@param ctrl? table
---@return FxRate
---@return string? err
function FxRateEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch FxRateListMatch
---@param ctrl? table
---@return FxRate[]
---@return string? err
function FxRateEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function FxRateEntity:_run_op(ctx, post_done)
  local utility = self._utility
