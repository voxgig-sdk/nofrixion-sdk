...
local RuleEventEntity = {}
RuleEventEntity.__index = RuleEventEntity


function RuleEventEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RuleEventEntity)
  self._name = "rule_event"
  self._client = client
...
function RuleEventEntity:get_name()
  return self._name
end


function RuleEventEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RuleEventEntity.new(self._client, opts)
end
...
function RuleEventEntity:mark_deleted()
  self._deleted = true
end


function RuleEventEntity:deleted()
  return true == self._deleted
end


function RuleEventEntity:data_set(args)
  if args ~= nil then
...
function RuleEventEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RuleEventEntity:match_set(args)
  if args ~= nil then
...
function RuleEventEntity:match_get()
...
function RuleEventEntity:stream(action, args, callopts)
...
---@param reqmatch RuleEventListMatch
---@param ctrl? table
---@return RuleEvent[]
---@return string? err
function RuleEventEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RuleEventEntity:_run_op(ctx, post_done)
  local utility = self._utility
