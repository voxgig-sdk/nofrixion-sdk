-- Nofrixion SDK PaymentRequestMinimal entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentRequestMinimalEntity = {}
PaymentRequestMinimalEntity.__index = PaymentRequestMinimalEntity


function PaymentRequestMinimalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentRequestMinimalEntity)
  self._name = "payment_request_minimal"
  self._client = client
...
function PaymentRequestMinimalEntity:get_name()
  return self._name
end


function PaymentRequestMinimalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentRequestMinimalEntity.new(self._client, opts)
end
...
function PaymentRequestMinimalEntity:mark_deleted()
  self._deleted = true
end


function PaymentRequestMinimalEntity:deleted()
  return true == self._deleted
end


function PaymentRequestMinimalEntity:data_set(args)
  if args ~= nil then
...
function PaymentRequestMinimalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentRequestMinimalEntity:match_set(args)
  if args ~= nil then
...
function PaymentRequestMinimalEntity:match_get()
...
function PaymentRequestMinimalEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentRequestMinimalListMatch
---@param ctrl? table
---@return PaymentRequestMinimal[]
---@return string? err
function PaymentRequestMinimalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentRequestMinimalEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PaymentRequestMinimalEntity
