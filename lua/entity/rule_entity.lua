...
local RuleEntity = {}
RuleEntity.__index = RuleEntity


function RuleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RuleEntity)
  self._name = "rule"
  self._client = client
...
function RuleEntity:get_name()
  return self._name
end


function RuleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RuleEntity.new(self._client, opts)
end
...
function RuleEntity:mark_deleted()
  self._deleted = true
end


function RuleEntity:deleted()
  return true == self._deleted
end


function RuleEntity:data_set(args)
  if args ~= nil then
...
function RuleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RuleEntity:match_set(args)
  if args ~= nil then
...
function RuleEntity:stream(action, args, callopts)
...
---@param reqdata RuleUpdateData
---@param ctrl? table
---@return Rule
---@return string? err
function RuleEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch RuleRemoveMatch
---@param ctrl? table
---@return Rule
---@return string? err
function RuleEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function RuleEntity:_run_op(ctx, post_done)
  local utility = self._utility
