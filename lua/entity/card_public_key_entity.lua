-- Nofrixion SDK CardPublicKey entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CardPublicKeyEntity = {}
CardPublicKeyEntity.__index = CardPublicKeyEntity


function CardPublicKeyEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardPublicKeyEntity)
  self._name = "card_public_key"
  self._client = client
...
function CardPublicKeyEntity:get_name()
  return self._name
end


function CardPublicKeyEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardPublicKeyEntity.new(self._client, opts)
end
...
function CardPublicKeyEntity:mark_deleted()
  self._deleted = true
end


function CardPublicKeyEntity:deleted()
  return true == self._deleted
end


function CardPublicKeyEntity:data_set(args)
  if args ~= nil then
...
function CardPublicKeyEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardPublicKeyEntity:match_set(args)
  if args ~= nil then
...
function CardPublicKeyEntity:match_get()
...
function CardPublicKeyEntity:stream(action, args, callopts)
...
---@param reqmatch CardPublicKeyLoadMatch
---@param ctrl? table
---@return CardPublicKey
---@return string? err
function CardPublicKeyEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CardPublicKeyEntity:_run_op(ctx, post_done)
  local utility = self._utility
