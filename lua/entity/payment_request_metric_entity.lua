-- Nofrixion SDK PaymentRequestMetric entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentRequestMetricEntity = {}
PaymentRequestMetricEntity.__index = PaymentRequestMetricEntity


function PaymentRequestMetricEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentRequestMetricEntity)
  self._name = "payment_request_metric"
  self._client = client
...
function PaymentRequestMetricEntity:get_name()
  return self._name
end


function PaymentRequestMetricEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentRequestMetricEntity.new(self._client, opts)
end
...
function PaymentRequestMetricEntity:mark_deleted()
  self._deleted = true
end


function PaymentRequestMetricEntity:deleted()
  return true == self._deleted
end


function PaymentRequestMetricEntity:data_set(args)
  if args ~= nil then
...
function PaymentRequestMetricEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentRequestMetricEntity:match_set(args)
  if args ~= nil then
...
function PaymentRequestMetricEntity:match_get()
...
function PaymentRequestMetricEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentRequestMetricLoadMatch
---@param ctrl? table
---@return PaymentRequestMetric
---@return string? err
function PaymentRequestMetricEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentRequestMetricEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PaymentRequestMetricEntity
