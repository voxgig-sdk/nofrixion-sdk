-- Nofrixion SDK IPayment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IPaymentEntity = {}
IPaymentEntity.__index = IPaymentEntity


function IPaymentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IPaymentEntity)
  self._name = "i_payment"
  self._client = client
...
function IPaymentEntity:get_name()
  return self._name
end


function IPaymentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IPaymentEntity.new(self._client, opts)
end
...
function IPaymentEntity:mark_deleted()
  self._deleted = true
end


function IPaymentEntity:deleted()
  return true == self._deleted
end


function IPaymentEntity:data_set(args)
  if args ~= nil then
...
function IPaymentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IPaymentEntity:match_set(args)
  if args ~= nil then
...
function IPaymentEntity:match_get()
...
function IPaymentEntity:stream(action, args, callopts)
...
---@param reqdata IPaymentCreateData
---@param ctrl? table
---@return IPayment
---@return string? err
function IPaymentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function IPaymentEntity:_run_op(ctx, post_done)
  local utility = self._utility
