-- Nofrixion SDK PaymentAccountMinimal entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentAccountMinimalEntity = {}
PaymentAccountMinimalEntity.__index = PaymentAccountMinimalEntity


function PaymentAccountMinimalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentAccountMinimalEntity)
  self._name = "payment_account_minimal"
  self._client = client
...
function PaymentAccountMinimalEntity:get_name()
  return self._name
end


function PaymentAccountMinimalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentAccountMinimalEntity.new(self._client, opts)
end
...
function PaymentAccountMinimalEntity:mark_deleted()
  self._deleted = true
end


function PaymentAccountMinimalEntity:deleted()
  return true == self._deleted
end


function PaymentAccountMinimalEntity:data_set(args)
  if args ~= nil then
...
function PaymentAccountMinimalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentAccountMinimalEntity:match_set(args)
  if args ~= nil then
...
function PaymentAccountMinimalEntity:match_get()
...
function PaymentAccountMinimalEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentAccountMinimalListMatch
---@param ctrl? table
---@return PaymentAccountMinimal[]
---@return string? err
function PaymentAccountMinimalEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentAccountMinimalEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PaymentAccountMinimalEntity
