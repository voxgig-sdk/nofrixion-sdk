...
local OpenBankingEntity = {}
OpenBankingEntity.__index = OpenBankingEntity


function OpenBankingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OpenBankingEntity)
  self._name = "open_banking"
  self._client = client
...
function OpenBankingEntity:get_name()
  return self._name
end


function OpenBankingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OpenBankingEntity.new(self._client, opts)
end
...
function OpenBankingEntity:mark_deleted()
  self._deleted = true
end


function OpenBankingEntity:deleted()
  return true == self._deleted
end


function OpenBankingEntity:data_set(args)
  if args ~= nil then
...
function OpenBankingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OpenBankingEntity:match_set(args)
  if args ~= nil then
...
function OpenBankingEntity:match_get()
...
function OpenBankingEntity:stream(action, args, callopts)
...
---@param reqdata OpenBankingCreateData
---@param ctrl? table
---@return OpenBanking
---@return string? err
function OpenBankingEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch OpenBankingRemoveMatch
---@param ctrl? table
---@return OpenBanking
---@return string? err
function OpenBankingEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function OpenBankingEntity:_run_op(ctx, post_done)
  local utility = self._utility
