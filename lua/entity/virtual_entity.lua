...
function VirtualEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VirtualEntity)
  self._name = "virtual"
  self._client = client
...
function VirtualEntity:get_name()
  return self._name
end


function VirtualEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VirtualEntity.new(self._client, opts)
end
...
function VirtualEntity:mark_deleted()
  self._deleted = true
end


function VirtualEntity:deleted()
  return true == self._deleted
end


function VirtualEntity:data_set(args)
  if args ~= nil then
...
function VirtualEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VirtualEntity:match_set(args)
  if args ~= nil then
...
function VirtualEntity:match_get()
...
function VirtualEntity:stream(action, args, callopts)
...
---@param reqdata VirtualCreateData
---@param ctrl? table
---@return Virtual
---@return string? err
function VirtualEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata VirtualUpdateData
---@param ctrl? table
---@return Virtual
---@return string? err
function VirtualEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function VirtualEntity:_run_op(ctx, post_done)
  local utility = self._utility
