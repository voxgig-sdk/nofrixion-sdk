-- Nofrixion SDK PaymentInitiation entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentInitiationEntity = {}
PaymentInitiationEntity.__index = PaymentInitiationEntity


function PaymentInitiationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentInitiationEntity)
  self._name = "payment_initiation"
  self._client = client
...
function PaymentInitiationEntity:get_name()
  return self._name
end


function PaymentInitiationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentInitiationEntity.new(self._client, opts)
end
...
function PaymentInitiationEntity:mark_deleted()
  self._deleted = true
end


function PaymentInitiationEntity:deleted()
  return true == self._deleted
end


function PaymentInitiationEntity:data_set(args)
  if args ~= nil then
...
function PaymentInitiationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentInitiationEntity:match_set(args)
  if args ~= nil then
...
function PaymentInitiationEntity:match_get()
...
function PaymentInitiationEntity:stream(action, args, callopts)
...
---@param reqdata PaymentInitiationCreateData
---@param ctrl? table
---@return PaymentInitiation
---@return string? err
function PaymentInitiationEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PaymentInitiationEntity:_run_op(ctx, post_done)
  local utility = self._utility
