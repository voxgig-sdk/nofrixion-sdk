-- Nofrixion SDK PaymentAccount entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PaymentAccountEntity = {}
PaymentAccountEntity.__index = PaymentAccountEntity


function PaymentAccountEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PaymentAccountEntity)
  self._name = "payment_account"
  self._client = client
...
function PaymentAccountEntity:get_name()
  return self._name
end


function PaymentAccountEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PaymentAccountEntity.new(self._client, opts)
end
...
function PaymentAccountEntity:mark_deleted()
  self._deleted = true
end


function PaymentAccountEntity:deleted()
  return true == self._deleted
end


function PaymentAccountEntity:data_set(args)
  if args ~= nil then
...
function PaymentAccountEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PaymentAccountEntity:match_set(args)
  if args ~= nil then
...
function PaymentAccountEntity:match_get()
...
function PaymentAccountEntity:stream(action, args, callopts)
...
---@param reqmatch PaymentAccountListMatch
---@param ctrl? table
---@return PaymentAccount[]
---@return string? err
function PaymentAccountEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PaymentAccountEntity:_run_op(ctx, post_done)
  local utility = self._utility
