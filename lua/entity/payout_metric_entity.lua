-- Nofrixion SDK PayoutMetric entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PayoutMetricEntity = {}
PayoutMetricEntity.__index = PayoutMetricEntity


function PayoutMetricEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PayoutMetricEntity)
  self._name = "payout_metric"
  self._client = client
...
function PayoutMetricEntity:get_name()
  return self._name
end


function PayoutMetricEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PayoutMetricEntity.new(self._client, opts)
end
...
function PayoutMetricEntity:mark_deleted()
  self._deleted = true
end


function PayoutMetricEntity:deleted()
  return true == self._deleted
end


function PayoutMetricEntity:data_set(args)
  if args ~= nil then
...
function PayoutMetricEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PayoutMetricEntity:match_set(args)
  if args ~= nil then
...
function PayoutMetricEntity:match_get()
...
function PayoutMetricEntity:stream(action, args, callopts)
...
---@param reqmatch PayoutMetricLoadMatch
---@param ctrl? table
---@return PayoutMetric
---@return string? err
function PayoutMetricEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PayoutMetricEntity:_run_op(ctx, post_done)
  local utility = self._utility
