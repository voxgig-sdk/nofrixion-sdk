-- Nofrixion SDK PayoutKeysetPage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PayoutKeysetPageEntity = {}
PayoutKeysetPageEntity.__index = PayoutKeysetPageEntity


function PayoutKeysetPageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PayoutKeysetPageEntity)
  self._name = "payout_keyset_page"
  self._client = client
...
function PayoutKeysetPageEntity:get_name()
  return self._name
end


function PayoutKeysetPageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PayoutKeysetPageEntity.new(self._client, opts)
end
...
function PayoutKeysetPageEntity:mark_deleted()
  self._deleted = true
end


function PayoutKeysetPageEntity:deleted()
  return true == self._deleted
end


function PayoutKeysetPageEntity:data_set(args)
  if args ~= nil then
...
function PayoutKeysetPageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PayoutKeysetPageEntity:match_set(args)
  if args ~= nil then
...
function PayoutKeysetPageEntity:match_get()
...
function PayoutKeysetPageEntity:stream(action, args, callopts)
...
---@param reqmatch PayoutKeysetPageListMatch
---@param ctrl? table
---@return PayoutKeysetPage[]
---@return string? err
function PayoutKeysetPageEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function PayoutKeysetPageEntity:_run_op(ctx, post_done)
  local utility = self._utility
