...
describe("OpenBankingEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:OpenBanking(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = open_banking_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "open_banking." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_OPEN_BANKING_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local open_banking_ref01_ent = client:OpenBanking(nil)
    local open_banking_ref01_data = helpers.to_map(vs.getprop(
...
function open_banking_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/open_banking/OpenBankingTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read open_banking test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_OPEN_BANKING_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["NOFRIXION_TEST_OPEN_BANKING_ENTID"])
