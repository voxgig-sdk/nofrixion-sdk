...
describe("CardCustomerTokenEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:CardCustomerToken(nil)
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CARD_CUSTOMER_TOKEN_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local card_customer_token_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.card_customer_token")))
    local card_customer_token_ref01_data = nil
    if #card_customer_token_ref01_data_raw > 0 then
      card_customer_token_ref01_data = helpers.to_map(card_customer_token_ref01_data_raw[1][2])
    end

    -- LIST
    local card_customer_token_ref01_ent = client:CardCustomerToken(nil)
    local card_customer_token_ref01_match = {
      ["customer_email_address"] = setup.idmap["customer_email_address01"],
      ["merchant_id"] = setup.idmap["merchant01"],
    }

    local card_customer_token_ref01_list_result, err = card_customer_token_ref01_ent:list(card_customer_token_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(card_customer_token_ref01_list_result)

    -- LOAD
    local card_customer_token_ref01_match_dt0 = {
...
function card_customer_token_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/card_customer_token/CardCustomerTokenTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read card_customer_token test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_CARD_CUSTOMER_TOKEN_ENTID")
