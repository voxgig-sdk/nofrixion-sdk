...
describe("BeneficiaryDirect", function()
  it("should direct-list-beneficiary", function()
    local setup = beneficiary_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-beneficiary", setup.live and "live" or "unit")
  ...
  it("should direct-load-beneficiary", function()
    local setup = beneficiary_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-beneficiary", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function beneficiary_direct_setup(mockres)
