...
describe("CardEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CARD_ENTID JSON to run live")
      return
    end
...
function card_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "card01", "card02", "card03", "paymentrequest01", "paymentrequest02", "paymentrequest03" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_CARD_ENTID")
