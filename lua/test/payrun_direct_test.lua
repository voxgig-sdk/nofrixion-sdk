...
describe("PayrunDirect", function()
  it("should direct-list-payrun", function()
    local setup = payrun_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-payrun", setup.live and "live" or "unit")
  ...
  it("should direct-load-payrun", function()
    local setup = payrun_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-payrun", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function payrun_direct_setup(mockres)
