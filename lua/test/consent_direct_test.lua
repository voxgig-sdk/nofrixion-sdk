...
describe("ConsentDirect", function()
  it("should direct-list-consent", function()
    local setup = consent_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-consent", setup.live and "live" or "unit")
  ...
  it("should direct-load-consent", function()
    local setup = consent_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-consent", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function consent_direct_setup(mockres)
