...
describe("BeneficiaryEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Beneficiary(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Beneficiary(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Beneficiary(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_BENEFICIARY_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local beneficiary_ref01_ent = client:Beneficiary(nil)
    local beneficiary_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LOAD
    local beneficiary_ref01_match_dt0 = {
      id = beneficiary_ref01_data["id"],
    }
    ...
    -- REMOVE
    local beneficiary_ref01_match_rm0 = {
      id = beneficiary_ref01_data["id"],
    }
...
function beneficiary_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/beneficiary/BeneficiaryTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read beneficiary test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_BENEFICIARY_ENTID")
