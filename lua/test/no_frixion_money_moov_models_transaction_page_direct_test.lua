-- NoFrixionMoneyMoovModelsTransactionPage direct test
...
describe("NoFrixionMoneyMoovModelsTransactionPageDirect", function()
  it("should direct-list-no_frixion_money_moov_models_transaction_page", function()
    local setup = no_frixion_money_moov_models_transaction_page_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-no_frixion_money_moov_models_transaction_page", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"account01"}) do
  ...
  it("should direct-load-no_frixion_money_moov_models_transaction_page", function()
    local setup = no_frixion_money_moov_models_transaction_page_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-no_frixion_money_moov_models_transaction_page", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function no_frixion_money_moov_models_transaction_page_direct_setup(mockres)
