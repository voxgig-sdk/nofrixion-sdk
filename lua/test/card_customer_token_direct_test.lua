...
describe("CardCustomerTokenDirect", function()
  it("should direct-list-card_customer_token", function()
    local setup = card_customer_token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-card_customer_token", setup.live and "live" or "unit")
  ...
  it("should direct-load-card_customer_token", function()
    local setup = card_customer_token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-card_customer_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function card_customer_token_direct_setup(mockres)
