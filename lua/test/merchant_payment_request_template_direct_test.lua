-- MerchantPaymentRequestTemplate direct test
...
describe("MerchantPaymentRequestTemplateDirect", function()
  it("should direct-list-merchant_payment_request_template", function()
    local setup = merchant_payment_request_template_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-merchant_payment_request_template", setup.live and "live" or "unit")
  ...
  it("should direct-load-merchant_payment_request_template", function()
    local setup = merchant_payment_request_template_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-merchant_payment_request_template", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function merchant_payment_request_template_direct_setup(mockres)
