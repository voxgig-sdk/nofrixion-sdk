...
describe("RuleEventDirect", function()
  it("should direct-list-rule_event", function()
    local setup = rule_event_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rule_event", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"rule_event01"}) do
...
function rule_event_direct_setup(mockres)
