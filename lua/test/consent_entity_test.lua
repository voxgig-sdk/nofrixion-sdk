...
describe("ConsentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Consent(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Consent(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Consent(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = consent_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "consent." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CONSENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local consent_ref01_ent = client:Consent(nil)
    local consent_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local consent_ref01_match = {
      ["email"] = setup.idmap["email01"],
      ["merchant_id"] = setup.idmap["merchant01"],
    }

    local consent_ref01_list_result, err = consent_ref01_ent:list(consent_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(consent_ref01_list_result)
    ...
    -- UPDATE
    local consent_ref01_data_up0_up = {
      id = consent_ref01_data["id"],
    }
    ...
    -- REMOVE
    local consent_ref01_match_rm0 = {
      id = consent_ref01_data["id"],
    }
    local _, err = consent_ref01_ent:remove(consent_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local consent_ref01_match_rt0 = {
      ["email"] = setup.idmap["email01"],
      ["merchant_id"] = setup.idmap["merchant01"],
    }

    local consent_ref01_list_rt0_result, err = consent_ref01_ent:list(consent_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(consent_ref01_list_rt0_result)
...
function consent_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/consent/ConsentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read consent test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_CONSENT_ENTID")
