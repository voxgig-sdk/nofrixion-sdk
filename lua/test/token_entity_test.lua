...
describe("TokenEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_TOKEN_ENTID JSON to run live")
      return
    end
    ...
    local token_ref01_data = helpers.to_map(vs.getprop(
    ...
    assert.is_not_nil(token_ref01_data["id"])
...
function token_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_TOKEN_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["NOFRIXION_TEST_TOKEN_ENTID"])
