...
describe("VirtualEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Virtual(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = virtual_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "update"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "virtual." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_VIRTUAL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local virtual_ref01_ent = client:Virtual(nil)
    local virtual_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- UPDATE
    local virtual_ref01_data_up0_up = {
...
function virtual_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/virtual/VirtualTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read virtual test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_VIRTUAL_ENTID")
