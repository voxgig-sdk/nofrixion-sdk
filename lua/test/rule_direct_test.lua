...
describe("RuleDirect", function()
  it("should direct-list-rule", function()
    local setup = rule_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-rule", setup.live and "live" or "unit")
  ...
  it("should direct-load-rule", function()
    local setup = rule_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-rule", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function rule_direct_setup(mockres)
