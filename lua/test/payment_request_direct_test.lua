...
describe("PaymentRequestDirect", function()
  it("should direct-list-payment_request", function()
    local setup = payment_request_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-payment_request", setup.live and "live" or "unit")
  ...
  it("should direct-load-payment_request", function()
    local setup = payment_request_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-payment_request", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function payment_request_direct_setup(mockres)
