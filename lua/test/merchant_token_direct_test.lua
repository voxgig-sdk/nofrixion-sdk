...
describe("MerchantTokenDirect", function()
  it("should direct-list-merchant_token", function()
    local setup = merchant_token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-merchant_token", setup.live and "live" or "unit")
  ...
  it("should direct-load-merchant_token", function()
    local setup = merchant_token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-merchant_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function merchant_token_direct_setup(mockres)
