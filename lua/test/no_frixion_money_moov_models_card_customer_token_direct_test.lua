-- NoFrixionMoneyMoovModelsCardCustomerToken direct test
...
describe("NoFrixionMoneyMoovModelsCardCustomerTokenDirect", function()
  it("should direct-list-no_frixion_money_moov_models_card_customer_token", function()
    local setup = no_frixion_money_moov_models_card_customer_token_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-no_frixion_money_moov_models_card_customer_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"customer_email_address01", "merchant01"}) do
  ...
  it("should direct-load-no_frixion_money_moov_models_card_customer_token", function()
    local setup = no_frixion_money_moov_models_card_customer_token_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-no_frixion_money_moov_models_card_customer_token", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function no_frixion_money_moov_models_card_customer_token_direct_setup(mockres)
