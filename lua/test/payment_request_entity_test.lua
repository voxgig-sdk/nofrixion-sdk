...
describe("PaymentRequestEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:PaymentRequest(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:PaymentRequest(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:PaymentRequest(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_PAYMENT_REQUEST_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    payment_request_ref01_data = helpers.to_map(type(payment_request_ref01_data_result) == 'table' and payment_request_ref01_data_result.data_get and payment_request_ref01_data_result:data_get() or payment_request_ref01_data_result)
    ...
    local payment_request_ref01_markdef_up0_name = "baseOriginUrl"
    ...
    local payment_request_ref01_resdata_up0_result, err = payment_request_ref01_ent:update(payment_request_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local payment_request_ref01_resdata_up0 = helpers.to_map(type(payment_request_ref01_resdata_up0_result) == 'table' and payment_request_ref01_resdata_up0_result.data_get and payment_request_ref01_resdata_up0_result:data_get() or payment_request_ref01_resdata_up0_result)
    ...
    assert.is_nil(err)
    local payment_request_ref01_data_dt0_load_result = helpers.to_map(type(payment_request_ref01_data_dt0_loaded) == 'table' and payment_request_ref01_data_dt0_loaded.data_get and payment_request_ref01_data_dt0_loaded:data_get() or payment_request_ref01_data_dt0_loaded)
...
function payment_request_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/payment_request/PaymentRequestTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read payment_request test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_PAYMENT_REQUEST_ENTID")
