...
describe("PayoutDirect", function()
  it("should direct-list-payout", function()
    local setup = payout_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-payout", setup.live and "live" or "unit")
  ...
  it("should direct-load-payout", function()
    local setup = payout_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-payout", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function payout_direct_setup(mockres)
