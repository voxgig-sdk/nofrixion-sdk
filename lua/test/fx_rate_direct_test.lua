...
describe("FxRateDirect", function()
  it("should direct-list-fx_rate", function()
    local setup = fx_rate_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-fx_rate", setup.live and "live" or "unit")
  ...
  it("should direct-load-fx_rate", function()
    local setup = fx_rate_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-fx_rate", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function fx_rate_direct_setup(mockres)
