...
describe("MandateEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Mandate(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = mandate_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "mandate." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_MANDATE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local mandate_ref01_ent = client:Mandate(nil)
    local mandate_ref01_data = helpers.to_map(vs.getprop(
...
function mandate_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/mandate/MandateTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read mandate test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("NOFRIXION_TEST_MANDATE_ENTID")
