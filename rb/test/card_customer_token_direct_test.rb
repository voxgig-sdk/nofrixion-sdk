...
class CardCustomerTokenDirectTest < Minitest::Test
  def test_direct_list_card_customer_token
    setup = card_customer_token_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-card_customer_token", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_card_customer_token
    setup = card_customer_token_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-card_customer_token", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def card_customer_token_direct_setup(mockres)
