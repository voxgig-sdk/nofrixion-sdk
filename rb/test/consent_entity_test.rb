...
class ConsentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Consent(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Consent(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = consent_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "consent." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CONSENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    consent_ref01_ent = client.Consent(nil)
    ...
    consent_ref01_list_result = consent_ref01_ent.list(consent_ref01_match, nil)
    assert consent_ref01_list_result.is_a?(Array)
    ...
    consent_ref01_ent.remove(consent_ref01_match_rm0, nil)
    ...
    consent_ref01_list_rt0_result = consent_ref01_ent.list(consent_ref01_match_rt0, nil)
    assert consent_ref01_list_rt0_result.is_a?(Array)
...
def consent_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "consent", "ConsentTestData.json")
  ...
  entid_env_raw = ENV["NOFRIXION_TEST_CONSENT_ENTID"]
