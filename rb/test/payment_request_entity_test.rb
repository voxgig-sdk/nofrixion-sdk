...
class PaymentRequestEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.PaymentRequest(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.PaymentRequest(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_PAYMENT_REQUEST_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def payment_request_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "payment_request", "PaymentRequestTestData.json")
  ...
  entid_env_raw = ENV["NOFRIXION_TEST_PAYMENT_REQUEST_ENTID"]
