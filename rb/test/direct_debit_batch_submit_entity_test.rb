...
class DirectDebitBatchSubmitEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = direct_debit_batch_submit_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "direct_debit_batch_submit." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_DIRECT_DEBIT_BATCH_SUBMIT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    direct_debit_batch_submit_ref01_ent = client.DirectDebitBatchSubmit(nil)
...
def direct_debit_batch_submit_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "direct_debit_batch_submit", "DirectDebitBatchSubmitTestData.json")
  ...
  entid_env_raw = ENV["NOFRIXION_TEST_DIRECT_DEBIT_BATCH_SUBMIT_ENTID"]
