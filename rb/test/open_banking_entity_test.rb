...
class OpenBankingEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = open_banking_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "open_banking." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_OPEN_BANKING_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    open_banking_ref01_ent = client.OpenBanking(nil)
...
def open_banking_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "open_banking", "OpenBankingTestData.json")
  ...
  entid_env_raw = ENV["NOFRIXION_TEST_OPEN_BANKING_ENTID"]
