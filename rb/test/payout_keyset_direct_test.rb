...
class PayoutKeysetDirectTest < Minitest::Test
  def test_direct_list_payout_keyset
    setup = payout_keyset_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-payout_keyset", setup[:live] ? "live" : "unit")
    if _should_skip
...
def payout_keyset_direct_setup(mockres)
