...
class NoFrixionMoneyMoovModelsOpenBankingTransactionDirectTest < Minitest::Test
  def test_direct_list_no_frixion_money_moov_models_open_banking_transaction
    setup = no_frixion_money_moov_models_open_banking_transaction_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-no_frixion_money_moov_models_open_banking_transaction", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["account01", "no_frixion_money_moov_models_open_banking_transaction01"].each do |_live_key|
...
def no_frixion_money_moov_models_open_banking_transaction_direct_setup(mockres)
