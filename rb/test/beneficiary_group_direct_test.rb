...
class BeneficiaryGroupDirectTest < Minitest::Test
  def test_direct_list_beneficiary_group
    setup = beneficiary_group_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-beneficiary_group", setup[:live] ? "live" : "unit")
    if _should_skip
...
def beneficiary_group_direct_setup(mockres)
