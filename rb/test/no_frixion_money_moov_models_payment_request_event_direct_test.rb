...
class NoFrixionMoneyMoovModelsPaymentRequestEventDirectTest < Minitest::Test
  def test_direct_list_no_frixion_money_moov_models_payment_request_event
    setup = no_frixion_money_moov_models_payment_request_event_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-no_frixion_money_moov_models_payment_request_event", setup[:live] ? "live" : "unit")
    if _should_skip
...
def no_frixion_money_moov_models_payment_request_event_direct_setup(mockres)
