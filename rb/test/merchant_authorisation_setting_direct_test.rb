...
class MerchantAuthorisationSettingDirectTest < Minitest::Test
  def test_direct_list_merchant_authorisation_setting
    setup = merchant_authorisation_setting_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-merchant_authorisation_setting", setup[:live] ? "live" : "unit")
    if _should_skip
...
def merchant_authorisation_setting_direct_setup(mockres)
