...
class PaymentRequestEventDirectTest < Minitest::Test
  def test_direct_list_payment_request_event
    setup = payment_request_event_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-payment_request_event", setup[:live] ? "live" : "unit")
    if _should_skip
...
def payment_request_event_direct_setup(mockres)
