...
module NofrixionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NofrixionResponse.new({})
    ...
    if fetch_err
      ...
      response = NofrixionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NofrixionResponse.new(fetched)
    else
