...
class ReportResultEntity
  ...
  def make
    opts = @_entopts.dup
    ReportResultEntity.new(@_client, opts)
  end
  ...
  # @return [ReportResult, Hash] the current ReportResult data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of ReportResult fields)
  def match_get
  ...
  #
  # @param reqmatch [ReportResultLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ReportResult.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ReportResult, Hash] the loaded ReportResult; raises NofrixionError on failure
