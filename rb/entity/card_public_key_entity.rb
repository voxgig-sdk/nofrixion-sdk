...
class CardPublicKeyEntity
  ...
  def make
    opts = @_entopts.dup
    CardPublicKeyEntity.new(@_client, opts)
  end
  ...
  # @return [CardPublicKey, Hash] the current CardPublicKey data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of CardPublicKey fields)
  def match_get
  ...
  #
  # @param reqmatch [CardPublicKeyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CardPublicKey.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CardPublicKey, Hash] the loaded CardPublicKey; raises NofrixionError on failure
