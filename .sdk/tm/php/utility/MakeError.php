...
// Nofrixion SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class NofrixionMakeError
{
    public static function call(?NofrixionContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NofrixionContext([], null);
        }
        $op = $ctx->op ?? new NofrixionOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NofrixionResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NofrixionError) ? $err->msg : (string)$err;
        $msg = "NofrixionSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new NofrixionError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof NofrixionError) {
