...
[MCP](https://modelcontextprotocol.io) server exposing the Nofrixion SDK as
two agent tools — `nofrixion_list` and `nofrixion_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nofrixion-mcp)
make build
...
```jsonc
// nofrixion_list: first page of records
{ "entity": "account" }
{ "entity": "account", "query": { } }

// nofrixion_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nofrixion-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nofrixion \
     -- "$PWD"/dist/*/nofrixion-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nofrixion_list` and `nofrixion_load` tools now appear
   in new sessions. Ask the agent to *"list account using nofrixion"*
   and it calls `nofrixion_list` with `{"entity":"account"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `nofrixion_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nofrixion_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 49 supported entities (see below). |
...
The 49 entities valid as the `entity` argument:
