...
class NoFrixionMoneyMoovModelsOpenBankingTransactionDirectTest extends TestCase
{
    public function test_direct_list_no_frixion_money_moov_models_open_banking_transaction(): void
    {
        $setup = no_frixion_money_moov_models_open_banking_transaction_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-no_frixion_money_moov_models_open_banking_transaction", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["account01", "no_frixion_money_moov_models_open_banking_transaction01"] as $_liveKey) {
