...
class VirtualEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Virtual(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = virtual_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "virtual." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_VIRTUAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $virtual_ref01_ent = $client->Virtual(null);
...
function virtual_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/virtual/VirtualTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["virtual01", "virtual02", "virtual03", "account01", "account02", "account03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_VIRTUAL_ENTID");
