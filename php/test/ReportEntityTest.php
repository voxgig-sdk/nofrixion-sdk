...
class ReportEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_REPORT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertEquals($report_ref01_resdata_up0["id"], $report_ref01_data_up0_up["id"]);
...
function report_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["report01", "report02", "report03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_REPORT_ENTID");
