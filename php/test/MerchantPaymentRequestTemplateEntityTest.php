...
class MerchantPaymentRequestTemplateEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = NofrixionSDK::test(null, null);
        $ent = $testsdk->MerchantPaymentRequestTemplate(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = merchant_payment_request_template_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "merchant_payment_request_template." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_MERCHANT_PAYMENT_REQUEST_TEMPLATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $merchant_payment_request_template_ref01_data = null;
        if (count($merchant_payment_request_template_ref01_data_raw) > 0) {
            $merchant_payment_request_template_ref01_data = Helpers::to_map($merchant_payment_request_template_ref01_data_raw[0][1]);
        }

        // LIST
        $merchant_payment_request_template_ref01_ent = $client->MerchantPaymentRequestTemplate(null);
        $merchant_payment_request_template_ref01_match = [
            "merchant_id" => $setup["idmap"]["merchant01"],
        ];

        $merchant_payment_request_template_ref01_list_result = $merchant_payment_request_template_ref01_ent->list($merchant_payment_request_template_ref01_match, null);
        $this->assertIsArray($merchant_payment_request_template_ref01_list_result);
        ...
        $merchant_payment_request_template_ref01_ent->remove($merchant_payment_request_template_ref01_match_rm0, null);
        ...
        $merchant_payment_request_template_ref01_list_rt0_result = $merchant_payment_request_template_ref01_ent->list($merchant_payment_request_template_ref01_match_rt0, null);
        $this->assertIsArray($merchant_payment_request_template_ref01_list_rt0_result);
...
function merchant_payment_request_template_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/merchant_payment_request_template/MerchantPaymentRequestTemplateTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["merchant_payment_request_template01", "merchant_payment_request_template02", "merchant_payment_request_template03", "paymentrequest01", "paymentrequest02", "paymentrequest03", "merchant01"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_MERCHANT_PAYMENT_REQUEST_TEMPLATE_ENTID");
