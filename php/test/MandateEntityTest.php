...
class MandateEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = NofrixionSDK::test(null, null);
        $ent = $testsdk->Mandate(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = mandate_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "mandate." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_MANDATE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $mandate_ref01_ent = $client->Mandate(null);
...
function mandate_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/mandate/MandateTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["mandate01", "mandate02", "mandate03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_MANDATE_ENTID");
