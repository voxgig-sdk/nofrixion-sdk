...
class MerchantTokenEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = NofrixionSDK::test(null, null);
        $ent = $testsdk->MerchantToken(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->MerchantToken(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->MerchantToken(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = merchant_token_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "merchant_token." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_MERCHANT_TOKEN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $merchant_token_ref01_ent = $client->MerchantToken(null);
        ...
        $merchant_token_ref01_list_result = $merchant_token_ref01_ent->list($merchant_token_ref01_match, null);
        $this->assertIsArray($merchant_token_ref01_list_result);
...
function merchant_token_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/merchant_token/MerchantTokenTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["merchant_token01", "merchant_token02", "merchant_token03", "merchant01", "merchant02", "merchant03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_MERCHANT_TOKEN_ENTID");
