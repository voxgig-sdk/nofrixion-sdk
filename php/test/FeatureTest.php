...
class FtCtrl extends NofrixionControl
{
...
class FtClient
{
    ...
    private NofrixionUtility $utility;

    public function __construct(NofrixionUtility $utility, array $options)
    {
    ...
    public function get_utility(): NofrixionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NofrixionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NofrixionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NofrixionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NofrixionConfig::make_feature($name);
    ...
    private static function buildUrl(NofrixionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NofrixionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NofrixionResponse($fetched);
        }
    ...
    private function fail(NofrixionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NofrixionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
