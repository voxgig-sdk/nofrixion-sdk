...
class TagEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_TAG_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function tag_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["tag01", "tag02", "tag03", "merchant01", "merchant02", "merchant03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_TAG_ENTID");
