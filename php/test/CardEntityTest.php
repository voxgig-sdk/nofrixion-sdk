...
class CardEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOFRIXION_TEST_CARD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function card_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["card01", "card02", "card03", "paymentrequest01", "paymentrequest02", "paymentrequest03"] as $k) {
    ...
    $entid_env_raw = getenv("NOFRIXION_TEST_CARD_ENTID");
