...
class NofrixionNetsimFeature extends NofrixionBaseFeature
{
    ...
    private function _simulate(NofrixionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(NofrixionContext $ctx, array $applied): void
    {
        $client = $this->client;
